...
def get_decision_boundaries(tree, feature_num, class_num):
    """ 
    Args:
    ...
    def dfs(idx=0, lower=[-100] * feature_num, upper=[100] * feature_num):
...
class TransductiveBoxInference(snt.AbstractModule):
    """
    Assists in predicting the class label of a sample drawn from a Gaussian
    distribution by calculating the probability that it falls into a hyperbox
    with given bounds and making the transductive assumption that data in the
    box share a common class distribution. This calculates a vector,
    [P(A and B_i)]_i where A is the event of the Gaussian sample falling into
    the given box and B_i being that the sample belongs to class i and then
    sums over boxes to get class probabilities.
    """
...
if __name__ == '__main__':
    ...
    boxes = get_decision_boundaries(clf, feature_num=4, class_num=3)

    with tf.Session() as sess:
        mu = tf.random_normal([2, 4], mean=.8, stddev=.1)
        sigma = tf.ones([2, 4])
        lower_bounds = tf.placeholder(tf.float32)
        upper_bounds = tf.placeholder(tf.float32)
        values = tf.placeholder(tf.float32)
        out, upper_cdf, lower_cdf = gaussian_box_inference(
            mu, sigma, lower_bounds, upper_bounds, values)

        feed_dict = {
            lower_bounds: boxes[0],
            upper_bounds: boxes[1],
            values: boxes[2]
        }
        print("upper")
        upper = sess.run(upper_cdf, feed_dict)
        print(upper)
        print("lower")
        lower = sess.run(lower_cdf, feed_dict)
        print(lower)
        print("difference")
        print(upper - lower)

        y = sess.run(
            [out, lower_bounds, upper_bounds, lower_cdf, upper_cdf],
            feed_dict={
                lower_bounds: boxes[0],
                upper_bounds: boxes[1],
                values: boxes[2]
