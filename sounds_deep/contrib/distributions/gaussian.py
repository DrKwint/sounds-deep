...
def natural_to_standard(eta1, eta2, name='gauss_to_stndrd'):
    with tf.name_scope(name):
        ...
        sigma = tf.matrix_inverse(-2 * eta2 + tf.eye(D) * np.finfo(np.float32).eps)
...
def log_probability_nat(x, eta1, eta2, weights=None):
    ...
    with tf.name_scope('gaussian_logprob'):
        ...
        logprob += 1. / 4 * tf.einsum('nkdi,nkdi->nk', tf.matrix_solve(eta2 + tf.eye(D) * 1e-20, eta1, name='mu'),
                                      eta1)
        ...
        with tf.name_scope('log_sum_exp'):
            ...
            normalizer = tf.add(
                max_logprob,
                tf.log(tf.reduce_sum(tf.exp(logprob - max_logprob), axis=1, keepdims=True)))
...
def log_probability_nat_per_samp(x_samps, eta1, eta2):
    ...
    with tf.name_scope('log_prob_4d'):
        ...
        log_normal = tf.einsum('nksd,nksd->nks', tf.einsum('nkij,nksj->nksi', eta2, x_samps),
                               x_samps)
        ...
        log_normal += 1.0 / 4 * tf.einsum(
            'nkdi,nkd->nki', tf.matrix_solve(eta2, tf.expand_dims(eta1, axis=-1)), eta1)
